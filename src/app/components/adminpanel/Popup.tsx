...
const Popup = ({setOpenPopup, setUpdateTable}:PropsType)=>{
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        axios.put(`/api/edit_product/${productData._id}`, inputData).then(res => {
            makeToast("Product updated Succeessfully")
  ...
  return (
    <div className = " fixed top-0 left-0 w-full h-screen bd-[#000000070] grid place-items-center">
        <div className='bg-white w-[700px] py-8 rounded-lg text-center relative'>
          ...
          />
          <h2 className="2xl -mt-3"> Edit Product</h2>
          <form className="mt-6 w-fit space-y-4 mx-auto" onSubmit={handleSubmit}>
            <input 
            className="border black border-gray-500 outline-none px-4 py-2 rounded-lg w-fit"
            type="text"
            placeholder="Name"
